...
class FollowersViewModel @Inject constructor(
    ...
    private val _state = MutableStateFlow(FollowersViewState())
    val state: StateFlow<FollowersViewState> = _state.asStateFlow()
    ...
    fun loadFollowers(userId: String) {
        viewModelScope.launch {
            ...
            // Load followers
            getUserFollowersUseCase(userId).collect { resource ->
                when (resource) {
                    is Resource.Success -> {
                        resource.data?.let { followers ->
                            ...
                            loadFollowStatusForUsers(followers)
                        }
                    }
                    is Resource.Error -> {
                        _state.update {
